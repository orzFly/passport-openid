...
Strategy.prototype.authenticate = function(req, options) {
  ...
  var returnURL = options.callbackURL || this._returnURL;
  var realm = options.realm || this._realm;
  if (returnURL) {
    var parsed = url.parse(returnURL);
    if (!parsed.protocol) {
      // The callback URL is relative, resolve a fully qualified URL from the
      // URL of the originating request.
      returnURL = url.resolve(utils.originalURL(req, { proxy: this._trustProxy }), returnURL);
      realm = url.resolve(utils.originalURL(req, { proxy: this._trustProxy }), "/");
    }
  }

  this._relyingParty.realm = realm;
  this._relyingParty.returnUrl = returnURL;
  ...
  if (req.query && req.query['openid.mode']) {
    ...
    this._relyingParty.verifyAssertion(req.url, function(err, result) {
      ...
      profile.id = result.claimedIdentifier;
      profile._json = JSON.parse(JSON.stringify(profile));
